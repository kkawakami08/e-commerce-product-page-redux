"use client";
import { useState } from "react";
import ProductPrice from "./ProductPrice";
import Counter from "./Counter";
import { useDispatch } from "react-redux";
import { addToCart } from "../GlobalRedux/Features/cartSlice";

export default function ProductDescription({ product }) {
  const { name, description, price, id } = product;
  const imgURL = product.imagesURLs[0];
  const [quantity, setQuantity] = useState(1);
  const dispatch = useDispatch();
  return (
    <div className="p-5 flex flex-col gap-6 w-screen pb-36 md:w-1/2  ">
      <p className="font-bold text-primaryOrange tracking-widest ">
        SNEAKER COMPANY
      </p>
      <h2 className="font-bold text-3xl tracking-wider">{name}</h2>
      <p className="text-darkGrayishBlue leading-6">{description}</p>
      <ProductPrice price={price} />
      <div className="md:flex-row  flex flex-col gap-5">
        <Counter quantity={quantity} setQuantity={setQuantity} />
        <div
          className="w-full bg-primaryOrange hover:bg-primaryOrange/80 p-4 rounded-lg flex items-center gap-5 justify-center shadow-lg shadow-primaryOrange/20 hover:cursor-pointer md:w-2/3"
          onClick={() => {
            dispatch(addToCart(id, name, price, imgURL, quantity));
          }}
        >
          <svg
            width="22"
            height="20"
            xmlns="http://www.w3.org/2000/svg"
            className="fill-white"
          >
            <path
              d="M20.925 3.641H3.863L3.61.816A.896.896 0 0 0 2.717 0H.897a.896.896 0 1 0 0 1.792h1l1.031 11.483c.073.828.52 1.726 1.291 2.336C2.83 17.385 4.099 20 6.359 20c1.875 0 3.197-1.87 2.554-3.642h4.905c-.642 1.77.677 3.642 2.555 3.642a2.72 2.72 0 0 0 2.717-2.717 2.72 2.72 0 0 0-2.717-2.717H6.365c-.681 0-1.274-.41-1.53-1.009l14.321-.842a.896.896 0 0 0 .817-.677l1.821-7.283a.897.897 0 0 0-.87-1.114ZM6.358 18.208a.926.926 0 0 1 0-1.85.926.926 0 0 1 0 1.85Zm10.015 0a.926.926 0 0 1 0-1.85.926.926 0 0 1 0 1.85Zm2.021-7.243-13.8.81-.57-6.341h15.753l-1.383 5.53Z"
              fill-rule="nonzero"
            />
          </svg>
          <p className="font-bold text-white text-lg">Add to cart</p>
        </div>
      </div>
    </div>
  );
}
